...
| Mutual authentication with unique per-device identities | Design not published. The durable rule: **validate the credential, then separately authorise the peer** — the two are different questions | OPS | **[core]** |
...
| **Protocol extensions designed now, used later** | Interactive and bulk-transfer operations need shapes the first design does not have. Work them out before anything is deployed, so a later phase is not a protocol break across a live fleet | RUN, OPS | **[core]** |
...
|---|---|---|---|
| The protocol design (not published) | The first design covers request/response only | Interactive and bulk-transfer operations need additional shapes. **Extension, not contradiction — but must be designed before deployment** | RUN, OPS |
